...
--
-- Хост: 127.0.0.1
-- Время создания: Авг 03 2017 г., 16:55
...
('ru', NULL, 'vasya-pupkin@my-domain.com', NULL, 'my-domain.com', 'yes', '', 'smtp.gmail.com', '', '', 25, 'no', 'no', 5, 1, '/usr/sbin/sendmail', 1, 2, 0, 'no', 'yes', 'no', 'no', 'no', 'no', 'yes', 'Подписка на рассылку', 'Здравствуйте, %NAME%\r\n\r\nПолучение рассылки возможно после завершения этапа активации подписки. Чтобы активировать подписку, перейдите по следующей ссылке: %CONFIRM%\r\n\r\nЕсли Вы не производили подписку на данный email, просто проигнорируйте это письмо или перейдите по ссылке: %UNSUB%\r\n\r\nС уважением, \r\nадминистратор сайта %SERVER_NAME%', 'yes', 'Отписаться от рассылки: <a href=%UNSUB%>%UNSUB%</a>', 'no', 1, 100, 'bulk', NULL, 0, 'no', 'no', 'my-domain.com', 'keyprivate/.htkeyprivate', 'phpnewsletter', 'password', '', 'no', 7);
...
--
-- Структура таблицы `pnl_сustomheaders`
--

CREATE TABLE `pnl_сustomheaders` (
  `id` int(11) NOT NULL,
...
--
-- Индексы таблицы `pnl_сustomheaders`
--
ALTER TABLE `pnl_сustomheaders`
  ADD PRIMARY KEY (`id`);
...
--
ALTER TABLE `pnl_template`
  MODIFY `id_template` int(9) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
...
--
-- AUTO_INCREMENT для таблицы `pnl_сustomheaders`
--
ALTER TABLE `pnl_сustomheaders`
