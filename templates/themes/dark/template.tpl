...
<div class="alert alert-danger">
  ...
  <span class="icon icon-remove-sign"></span> <strong>${STR_ERROR}!</strong> ${ERROR_ALERT} </div>
<!-- END IF -->
<form action="${ACTION}" onSubmit="if(document.forms[0].action.value == 0){window.alert('${ALERT_SELECT_ACTION}');return false;}if(document.forms[0].action.value == 4){return confirm('${ALERT_CONFIRM_REMOVE}');} if(document.forms[0].action.value == 1) return false" method="post">
  <table class="table table-hover table-bordered">
    <thead>
      <tr>
        <th class="center"><input type="checkbox" title="TABLECOLMN_CHECK_ALLBOX" onclick="CheckAll_Activate(this,'activate[]');"></th>
        <th width="5%">ID</th>
    ...
    <tbody>
      ...
      <tr class="td-middle${CLASS_NOACTIVE}">
        <td class="center"><input type="checkbox" onclick="Count_checked();" title="${TABLECOLMN_CHECKBOX}" value="${ROW_ID_TEMPLATE}" name=activate[]></td>
        <td class="center">${ROW_ID_TEMPLATE}</td>
        <td><a title="${STR_EDIT_MAILINGTEXT}" href="./?t=edit_template&id_template=${ROW_ID_TEMPLATE}">${ROW_TMPLNAME}</a><br>
          <br>
          ${ROW_CONTENT}
          </div></td>
        <td class="center">${ROW_CATNAME}</td>
        <td class="center">${ROW_ACTIVE}</td>
        <td class="center col-xs-1"><a href="./?id_template=${ROW_ID_TEMPLATE}&pos=up" class="btn btn-default" title="${STR_DOWN}"><i class="fa fa-sort-asc"></i></a>
          <div style="padding: 5px;">
            <input class="form-control" type="text" name="pos" value="${ROW_POS}">
          </div>
          <a href="./?id_template=${ROW_ID_TEMPLATE}&pos=down" class="btn btn-default" title="${STR_DOWN}"> <i class="fa fa-sort-desc"></i></a>
          </div></td>
        <td class="center"><a href="./?t=edit_template&id_template=${ROW_ID_TEMPLATE}" class="btn btn-default btn" title="${STR_EDIT}"><i class="icon icon-edit"></i></a></td>
      </tr>
      <!-- END column -->
      <!-- END row -->
    </tbody>
